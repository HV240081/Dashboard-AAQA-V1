...
let UploadService = class UploadService {
    ...
    permissionKeyForCategory(category) {
        ...
        if (normalized === 'community' || normalized === 'dc' || normalized.includes('desarrollo'))
            return 'DC';
        if (normalized === 'fis' || normalized === 'es' || normalized.includes('emprendimiento'))
            return 'ES';
        if (normalized.includes('corte') || normalized.includes('adesco') || normalized.includes('formacion'))
            return 'formacion_dc';
        if (normalized === 'ong')
            return 'ONG';
        return category;
    ...
    canUserEditCategory(user, category) {
        if (!user)
            return false;
        if (user.rol_id === 1 || user.rolId === 1 || user.rol_id === 3 || user.rolId === 3 || user.canEditAll === true)
            return true;
    ...
    async recalculateProjectProgress(projectId) {
        ...
        const activities = await this.entityManager.query(`SELECT nombre as name, mes as month, estado as status
             FROM proyecto_actividades
             WHERE proyecto_id = ?
             ORDER BY mes ASC, created_at ASC`, [projectId]);
        const reports = await this.entityManager.query(`SELECT mes, COALESCE(anio, año) as year, meta_financiera as metaFinancial, estado_cronograma as scheduleStatus
             FROM reportes_mensuales
             WHERE proyecto_id = ? AND mes BETWEEN 1 AND 12
             ORDER BY COALESCE(anio, año) ASC, mes ASC`, [projectId]);
    ...
    extractWorkbookProjectContext(workbook) {
        ...
        for (const sheetName of candidateSheets) {
            ...
            for (const row of rows) {
                ...
                if (['nombredelproyecto', 'mesdelproyecto', 'estado', 'estadodelmes', 'actividad', 'metafinanciera', 'observacionbreve', 'observaciones'].includes(key)
            ...
            if (monthlySheetName) {
                ...
                const processFlatMonthlyRows = async () => {
                    ...
                    headerRow.forEach((cell, index) => {
                        ...
                        if (normalizedCell)
                            headerIndexByName.set(normalizedCell, index);
                    });
                    ...
                    const resolveRowValue = (rowValues, headerKeys, fallbackIndex = null) => {
                        for (const headerKey of headerKeys) {
                            ...
                            if (index !== undefined)
                                return isBlank(rowValues[index]) ? null : rowValues[index];
                        }
                    ...
                    for (let i = headerRowIndex + 1; i < oa.length; i++) {
                        ...
                        if (rawRow.every((cell) => isBlank(cell)))
                            continue;
                        ...
                        if (isHeaderLike(projectCell) || isHeaderLike(monthCell) || isHeaderLike(activityCell))
                            continue;
                        if (projectCell) {
                        ...
                        if (monthCell)
                            currentMonthLabel = monthCell;
                        if (!currentProjectName && !activityCell)
                            continue;
                        ...
                        if (statusCell && !isHeaderLike(statusCell))
                            group.status = statusCell;
                        if (!isBlank(metaCell)) {
                    ...
                    for (const group of groups.values()) {
                        ...
                        try {
                            ...
                            const projectMeta = await this.entityManager.query(`SELECT p.nombre_proyecto as projectName, p.mes_inicio as startMonth, p.mes_final as endMonth, e.año as projectYear, p.monto_fgk as amountFGK, p.meta_financiera as metaFinancial
                                 FROM proyectos p
                                 LEFT JOIN ediciones e ON p.edicion_id = e.id
                                 WHERE p.id = ?
                                 LIMIT 1`, [resolvedProject.id]);
                            ...
                            const existingReport = await this.entityManager.query(`SELECT meta_financiera as metaFinancial, estado_cronograma as scheduleStatus, observaciones as observations
                                 FROM reportes_mensuales
                                 WHERE id = ? OR (proyecto_id = ? AND mes = ? AND COALESCE(anio, año) = ?)
                                 ORDER BY created_at DESC
                                 LIMIT 1`, [reportId, resolvedProject.id, reportMonth, reportYear]);
                            ...
                            const existingAccumulated = await this.entityManager.query(`SELECT COALESCE(SUM(meta_financiera), 0) as total
                                 FROM reportes_mensuales
                                 WHERE proyecto_id = ? AND id <> ?`, [resolvedProject.id, reportId]);
                            ...
                            await this.entityManager.query(`UPDATE proyecto_actividades
                                 SET estado = ?,
                                     observaciones = NULL,
                                     source = 'excel',
                                     manual_locked = 0,
    ...
    async cleanupOrphanFollowUpData() {
        ...
        for (const item of deleteStatements) {
            const result = await this.entityManager.query(`DELETE ${item.alias}
    ...
    async processCommunityCortesExcel(file, user) {
        ...
        try {
            ...
            for (const sheetName of workbook.SheetNames) {
                const flatRows = this.extractSheetRowsWithHeaders(workbook.Sheets[sheetName], ['Nombre del Corte', 'Nombre de ADESCO', 'Nombre Completo']);
        ...
        try {
            ...
            await this.entityManager.transaction(async (transactionalEntityManager) => {
                ...
                for (const participant of participants) {
                    await transactionalEntityManager.query(`INSERT INTO participantes_incubadora (id, ano, programa, sede, nombre, genero, edad, nombre_emprendimiento, departamento, estado, observaciones, created_at)
                         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())
                         ON DUPLICATE KEY UPDATE
                           ano = VALUES(ano),
    ...
    async processProjectRow(row, context = {}) {
        ...
        const timelineDurationFromExcel = (() => {
            if (timelineDurationRaw === null || timelineDurationRaw === undefined || timelineDurationRaw === '')
                return null;
    ...
    async processAllyRow(row, user = null, context = {}) {
        ...
        if (contextCategory) {
            queryStr += ` AND EXISTS (SELECT 1 FROM organizaciones o WHERE o.id = proyectos.organizacion_id AND o.categoria = ?)`;
    ...
    async processProjectActivitiesSheet(rows, user = null, context = {}) {
        ...
        for (const [projectId, activities] of groupedRows.entries()) {
            const projectMeta = await this.entityManager.query(`SELECT nombre_proyecto as projectName, mes_inicio as startMonth, mes_final as endMonth FROM proyectos WHERE id = ?`, [projectId]);
            ...
            for (const activity of activities) {
                ...
                const existingActivity = await this.entityManager.query(`SELECT id, estado as status, source, manual_locked FROM proyecto_actividades WHERE proyecto_id = ? AND mes = ? AND LOWER(nombre) = LOWER(?) LIMIT 1`, [projectId, activityMonth, activity.name]);
    ...
    async processSimplifiedMonthlyTrackingSheet(sheet, user = null, context = {}) {
        ...
        for (let i = 0; i < oa.length; i++) {
            ...
            const looksLikeProjectLabel = (firstCell.includes('nombredelproyecto') ||
                firstCell === 'nombreproyecto' ||
                firstCell === 'proyecto') && row[1] !== undefined && row[1] !== null && row[1] !== '';
        ...
        for (const [projectId, activities] of groupedRows.entries()) {
            const projectMeta = await this.entityManager.query(`SELECT p.nombre_proyecto as projectName, p.mes_inicio as startMonth, p.mes_final as endMonth, e.año as projectYear, p.monto_fgk as amountFGK, p.meta_financiera as metaFinancial FROM proyectos p LEFT JOIN ediciones e ON p.edicion_id = e.id WHERE p.id = ?`, [projectId]);
            const timeline = this.validateProjectTimeline(projectMeta, projectMeta[0]?.projectName || projectId);
            let index = 0;
            ...
            const technicalPercent = (() => {
                ...
                if (normalized) {
                    if (normalized.includes('finaliz') || normalized.includes('complet'))
                        return 100;
                    if (normalized.includes('ejec') || normalized.includes('proces') || normalized.includes('activ') || normalized.includes('avance'))
                        return 50;
    ...
    async processSimplifiedMonthlyTrackingBlocks(sheet, user = null, context = {}) {
        ...
        const allowOverwriteManual = ['true', '1', 'yes', 'si', 's�'].includes((context?.overwriteExisting ?? '').toString().trim().toLowerCase());
        const contextOrganization = this.pickValue(projectContext, ['organization', 'organizacion', 'organizaci�n'], '').toString().trim();
        const candidateProjects = await this.entityManager.query(`SELECT p.id, p.nombre_proyecto, o.nombre as organizacion_nombre, o.categoria as categoria_nombre, e.año as project_year
             FROM proyectos p
             INNER JOIN organizaciones o ON p.organizacion_id = o.id
             LEFT JOIN ediciones e ON p.edicion_id = e.id
             WHERE (? IS NULL OR o.categoria = ?)
               AND (? IS NULL OR e.año = ?)`, [contextCategory, contextCategory, contextYear, contextYear]).catch(() => []);
        const candidateByName = new Map();
        const normalizedName = (value) => (value ?? '').toString().trim().toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
        ...
        for (const block of blocks) {
            try {
                ...
                const projectMeta = await this.entityManager.query(`SELECT p.nombre_proyecto as projectName, p.mes_inicio as startMonth, p.mes_final as endMonth, e.año as projectYear, p.monto_fgk as amountFGK, p.meta_financiera as metaFinancial FROM proyectos p LEFT JOIN ediciones e ON p.edicion_id = e.id WHERE p.id = ?`, [resolvedProject.id]);
                ...
                for (const activity of block.activities) {
                    ...
                    const existingActivity = await this.entityManager.query(`SELECT id, estado as status, source, manual_locked FROM proyecto_actividades WHERE proyecto_id = ? AND mes = ? AND LOWER(nombre) = LOWER(?) LIMIT 1`, [resolvedProject.id, activityMonth, activity.name]);
                    if (existingActivity.length > 0 && Number(existingActivity[0].manual_locked) === 1 && (existingActivity[0].source || '').toString().toLowerCase() === 'manual') {
                        errors.push({
    ...
    async processMonthlyReportRow(row, user = null, context = {}) {
        ...
        if (proyectos.length === 0 && organizationName && contextCategory) {
            proyectos = await this.entityManager.query(`SELECT p.id, p.nombre_proyecto, o.nombre as organizacion_nombre, o.categoria as categoria_nombre
                 FROM proyectos p
                 INNER JOIN organizaciones o ON p.organizacion_id = o.id
                 WHERE o.categoria = ? AND LOWER(o.nombre) = LOWER(?)`, [contextCategory, organizationName]);
        }
        if (proyectos.length === 0) {
        ...
        if (activityName) {
            const activityStatus = (() => {
                ...
                if (normalizedSchedule.includes('finaliz') || normalizedSchedule.includes('complet'))
                    return 'completed';
                if (normalizedSchedule.includes('ejec') || normalizedSchedule.includes('proces') || normalizedSchedule.includes('activ') || normalizedSchedule.includes('avance') || normalizedSchedule.includes('tiempo'))
                    return 'active';
                if (normalizedSchedule.includes('pend'))
                    return 'pending';
                return 'pending';
            })();
...
exports.UploadService = UploadService;
exports.UploadService = exports.UploadService = UploadService = __decorate([
