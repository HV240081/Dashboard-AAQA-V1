...
let ReportsService = class ReportsService {
    ...
    buildBar(value, max, width = 18) {
        const safeMax = max > 0 ? max : 1;
        const ratio = Math.max(0, Math.min(1, value / safeMax));
        const filled = Math.max(1, Math.round(ratio * width));
        return '█'.repeat(filled).padEnd(width, '░');
    }
    async generateCategoryExcelNative(category, year) {
        ...
        try {
            const result = (0, node_child_process_1.spawnSync)(nodeExe, [scriptPath, inputFile, outputFile], {
                encoding: 'utf8',
                windowsHide: true,
            });
            if (result.status !== 0) {
    ...
    async runNativeExcelScript(scriptName, payload, prefix) {
    ...
    async getCurrentReportYear() {
        const current = await this.entityManager.query(`SELECT año AS year FROM ediciones WHERE es_actual = TRUE ORDER BY año DESC LIMIT 1`);
        if (current?.[0]?.year)
            return Number(current[0].year);
        const latest = await this.entityManager.query(`SELECT MAX(año) AS year FROM ediciones`);
        if (latest?.[0]?.year)
            return Number(latest[0].year);
        return new Date().getFullYear();
    }
    categoryTitle(category) {
        if (category === 'Community')
            return 'Desarrollo Comunitario';
        if (category === 'FIS')
            return 'Emprendimiento Social';
        return 'ONG';
    }
    ...
    async buildExecutiveReportPayload() {
        const currentYear = await this.getCurrentReportYear();
        const categories = ['ONG', 'Community', 'FIS'];
        const [globalHistorical, globalCurrent, categoriesHistorical, categoriesCurrent, formativeHistorical, formativeCurrent, allProjects, allCurrentProjects, cortes,] = await Promise.all([
        ...
        const categorySections = categories.map((category) => {
            ...
            const historicalProjects = allProjects.filter((project) => this.normalizeCategory(project.category) === category);
            const currentProjects = allCurrentProjects.filter((project) => this.normalizeCategory(project.category) === category);
            return {
                key: category,
                title: this.categoryTitle(category),
                historicalStats: categoriesHistorical[key],
                currentStats: categoriesCurrent[key],
                historicalProjects,
                currentProjects,
                participants: participantsByCategory[category] || [],
                formativeHistorical: formativeHistorical.byCategory?.[key] || { enrolled: 0, graduated: 0 },
                formativeCurrent: formativeCurrent.byCategory?.[key] || { enrolled: 0, graduated: 0 },
            };
        });
        return {
    ...
    async generateSummaryPdf() {
        {
            ...
            const write = (page, value, x, y, size = 10, font = regular, color = navy) => {
                page.drawText((value ?? '').toString().slice(0, 105), { x, y, size, font, color });
            };
            const card = (page, title, value, x, y, w, h, color = blue) => {
            ...
            const projectTable = (page, projects, startY) => {
            ...
            for (const section of payload.categorySections) {
                ...
                const currentAllies = section.currentProjects.reduce((acc, project) => acc + (Number(project.amountAllies) || 0), 0);
                const currentCounterpart = section.currentProjects.reduce((acc, project) => acc + (Number(project.counterpart) || 0), 0);
                const direct = section.currentProjects.reduce((acc, project) => acc + (Number(project.beneficiaries) || 0), 0);
                const indirect = section.currentProjects.reduce((acc, project) => acc + (Number(project.indirectBeneficiaries) || 0), 0);
                const techAvg = section.currentProjects.length
                    ? this.clampPercent(section.currentProjects.reduce((acc, project) => acc + (Number(project.technicalProgressPercentage) || 0), 0) / section.currentProjects.length)
                    : 0;
                const finAvg = section.currentProjects.length
                    ? this.clampPercent(section.currentProjects.reduce((acc, project) => acc + (Number(project.financialProgressPercentage) || 0), 0) / section.currentProjects.length)
                    : 0;
            ...
            const appendProjectAppendix = (title, projects) => {
    ...
    async generateCategoryExcel(category, year) {
        ...
        const currentProjectsByStatus = currentProjects.reduce((acc, p) => {
        ...
        const historicalRows = Array.from(historicalByYear.values())
            ...
            'Prom. Técnico': this.clampPercent(avg(item.technical, 'value') || (item.technical.reduce((a, b) => a + b, 0) / Math.max(1, item.technical.length))),
            'Prom. Financiero': this.clampPercent(avg(item.financial, 'value') || (item.financial.reduce((a, b) => a + b, 0) / Math.max(1, item.financial.length))),
        }));
        ...
        Object.entries(currentProjectsByStatus)
            .sort((a, b) => b[1] - a[1])
    ...
    async generateFormativeExcel() {
        ...
        const deptSheet = XLSX.utils.json_to_sheet(Object.entries(formativeData.byDepartment).map(([dept, count]) => ({
            Departamento: dept,
            Graduados: count,
        })));
    ...
    async getFormativeParticipants(filters) {
        let sql = `
      SELECT 
