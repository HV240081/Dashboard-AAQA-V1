...
export class ProjectsService implements OnModuleInit {
  ...
  private getCategoryCode(category: any): 'ONG' | 'DC' | 'FIS' {
    const normalized = this.normalizeCategory(category);
    if (normalized === 'Community') return 'DC';
    if (normalized === 'FIS') return 'FIS';
    return 'ONG';
  }

  private getProjectIdCandidates(projectId: any, category?: any): string[] {
  ...
  private normalizeScheduleStatus(value: any): 'pending' | 'active' | 'completed' {
    const status = (value ?? '').toString().trim().toLowerCase();
    if (status.includes('finaliz') || status === 'completed') return 'completed';
    if (status.includes('tiempo') || status.includes('ejec') || status.includes('adelant') || status.includes('retras') || status.includes('proceso') || status === 'active') return 'active';
    if (status.includes('pend') || status === 'pending') return 'pending';
    return 'pending';
  }

  private normalizeActivityStatus(value: any): 'pending' | 'active' | 'completed' {
    const status = (value ?? '').toString().trim().toLowerCase();
    if (status.includes('finaliz') || status.includes('complet')) return 'completed';
    if (status.includes('ejec') || status.includes('avance') || status.includes('activo') || status === 'active' || status.includes('proces')) return 'active';
    return 'pending';
  }

  private getMonthNumber(value: any): number | null {
  ...
  private buildTimelineMonthSequence(startMonth: number | null, durationMonths: number | null): number[] {
  ...
  private computeTimelineDurationMonths(startMonth: number | null, endMonth: number | null): number | null {
  ...
  private deriveTimelineEndMonth(startMonth: number | null, durationMonths: number | null): number | null {
  ...
  private normalizeMetaFinancial(value: any): number | null {
    if (value === null || value === undefined || value === '') return null;
  ...
  private validateMetaFinancialAgainstFgk(metaFinancial: any, fgkAmount: any, context = 'seguimiento') {
  ...
  private async ensureTimelineSchema() {
    ...
    `);
    const found = new Set(existingColumns.map((row: any) => row.COLUMN_NAME));
  ...
  private async ensureProjectCoreSchema() {
    const schemaColumns = [
    ...
    `);
    const found = new Map(existingColumns.map((row: any) => [row.COLUMN_NAME, row.IS_NULLABLE]));
  ...
  private async ensureProjectDocumentsSchema() {
  ...
  private async ensureProjectActivitiesSchema() {
    await this.entityManager.query(`
    ...
    `);
    if (!existingColumns.some((row: any) => row.COLUMN_NAME === 'observaciones')) {
    ...
    `);
    if (!sourceColumns.some((row: any) => row.COLUMN_NAME === 'source')) {
    ...
    `);
    if (!manualLockedColumns.some((row: any) => row.COLUMN_NAME === 'manual_locked')) {
  ...
  private async ensureProjectCommentsSchema() {
    await this.entityManager.query(`
  ...
  private async ensureMonthlyReportsSchema() {
    const existingColumns = await this.entityManager.query(`
      SELECT COLUMN_NAME
      FROM INFORMATION_SCHEMA.COLUMNS
    ...
    `);
    if (!yearColumns.some((row: any) => row.COLUMN_NAME === 'anio')) {
    ...
    if (!existingColumns.some((row: any) => row.COLUMN_NAME === 'meta_financiera')) {
  ...
  private splitUrls(value: any): string[] {
  ...
  private async syncProjectPhotos(projectId: string, photos: string[] = [], userId?: number) {
    ...
    for (const url of photos.filter(Boolean)) {
      await this.entityManager.query(
        `INSERT INTO fotos_proyecto (id, proyecto_id, url, subido_por, created_at)
         VALUES (?, ?, ?, ?, NOW())`,
        [`foto-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`, projectId, url, userId || null]
      );
    }
  }

  private async syncProjectDocuments(projectId: string, completionLetter: any, changeControlDocuments: any, monthlyTrackingDocuments: any, userId?: number) {
    await this.entityManager.query(
      `DELETE FROM proyecto_documentos WHERE proyecto_id = ? AND tipo IN ('completion_letter', 'change_control', 'monthly_tracking')`,
      [projectId]
    );
    ...
    for (const url of completionUrls) {
      await this.entityManager.query(
        `INSERT INTO proyecto_documentos (id, proyecto_id, tipo, url, subido_por, created_at)
         VALUES (?, ?, 'completion_letter', ?, ?, NOW())`,
        [`doc-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`, projectId, url, userId || null]
      );
    }
  ...
  private async recalculateProjectProgress(projectId: string) {
    ...
    const reports = await this.entityManager.query(
      `SELECT mes, COALESCE(anio, año) as year, avance_tecnico_real as realTechnical, avance_financiero_real as realFinancial, meta_financiera as metaFinancial, estado_cronograma as scheduleStatus
       FROM reportes_mensuales
       WHERE proyecto_id = ? AND mes BETWEEN 1 AND 12
       ORDER BY COALESCE(anio, año) DESC, mes ASC`,
      [projectId]
    );

    const timeline = Array(12).fill('pending');
    const statusToNormalized = (value: any) => {
      const status = (value ?? '').toString().trim().toLowerCase();
      if (status.includes('finaliz') || status === 'completed') return 'completed';
      if (status.includes('proceso') || status.includes('ejec') || status.includes('avance') || status === 'active') return 'active';
      if (status.includes('pend') || status === 'pending') return 'pending';
      return 'pending';
    };
    ...
    if (reports.length > 0) {
      for (const slot of timelineSlots) {
        const matchingReport = [...reports].reverse().find((report: any) => {
    ...
    if (reports.length > 0) {
      ...
      const groupedByMonth = new Map<number, any[]>();
      for (const activity of activities) {
        const month = this.getMonthNumber(activity.month) || 1;
    ...
    if (reports.length > 0) {
      ...
      if (totalExecutedAmount > 0) {
        const amountRow = await this.entityManager.query(
          `SELECT monto_fgk as amountFGK FROM proyectos WHERE id = ? LIMIT 1`,
          [projectId]
        );
  ...
  private async findOrCreateOrganization(orgName: string, category: any): Promise<string> {
    const normalizedName = (orgName ?? '').toString().trim();
    const normalizedCategory = this.normalizeCategory(category);
    const existing = await this.entityManager.query(
      `SELECT id FROM organizaciones WHERE nombre = ? AND categoria = ?`,
    ...
    if (existing.length > 0) {
      ...
      await this.entityManager.query(
        `UPDATE organizaciones SET categoria = ?, tipo_organizacion = ? WHERE id = ?`,
        [normalizedCategory, normalizedCategory === 'Community' ? 'ADESCO' : 'ONG', orgId]
      );
      return orgId;
    }
    ...
    await this.entityManager.query(
      `INSERT INTO organizaciones (id, nombre, categoria, tipo_organizacion) VALUES (?, ?, ?, ?)`,
      [newOrgId, normalizedName, normalizedCategory, normalizedCategory === 'Community' ? 'ADESCO' : 'ONG']
    );
    return newOrgId;
  }

  // ==============================================
  // HELPER: GENERAR ID �NICO YYYYXX
  // ==============================================
  async generateCustomProjectId(year: number, category: any, manager?: EntityManager): Promise<string> {
    ...
    for (const num of existingNumbers) {
      if (num === nextNum) {
        nextNum++;
      } else if ((num as number) > nextNum) {
        break;
      }
    }
    
    // Formatear con ceros a la izquierda (ej. 01, 02)
  ...
  // ==============================================
  // M�TODOS PRINCIPALES CRUD
  // ==============================================

  async updateProject(id: string, projectData: any, userId: number) {
    console.log(`?? Actualizando proyecto ${id} por usuario ${userId}`);
    
    const existing = await this.entityManager.query(
      `SELECT id, organizacion_id, edicion_id, meta_financiera FROM proyectos WHERE id = ?`,
      [id]
    );
    ...
    if (projectData.organization) {
      ...
      const organizacion = await this.entityManager.query(
        `SELECT id FROM organizaciones WHERE nombre = ? AND categoria = ?`,
        [orgName, targetCategory]
      );
      if (organizacion.length === 0) {
        const newOrgId = `org-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`;
        await this.entityManager.query(
          `INSERT INTO organizaciones (id, nombre, categoria, tipo_organizacion) VALUES (?, ?, ?, ?)`,
          [newOrgId, orgName, targetCategory, targetCategory === 'Community' ? 'ADESCO' : 'ONG']
        );
        ...
        await this.entityManager.query(
          `UPDATE organizaciones SET categoria = ?, tipo_organizacion = ? WHERE id = ?`,
          [targetCategory, targetCategory === 'Community' ? 'ADESCO' : 'ONG', organizacionId]
        );
      }
    }
    ...
    if (projectData.year !== undefined) {
      ...
      const edicion = await this.entityManager.query(
        `SELECT id FROM ediciones WHERE año = ? LIMIT 1`,
        [targetYear]
      );
      if (edicion.length === 0) {
    ...
    if (projectData.completionLetter !== undefined || projectData.changeControlDocuments !== undefined || projectData.monthlyTrackingDocuments !== undefined) {
      await this.syncProjectDocuments(
        id,
        projectData.completionLetter,
        projectData.changeControlDocuments,
        projectData.monthlyTrackingDocuments,
        userId
      );
    }
    ...
    if (projectData.trainingDetails && Array.isArray(projectData.trainingDetails.participants)) {
      ...
      await this.entityManager.query(
        `DELETE FROM participantes_formacion WHERE proyecto_id IN (${projectIdPlaceholders})`,
        projectIdCandidates
      );
    ...
    if (projectData.allies && projectData.allies.length > 0) {
      ...
      const projectIdPlaceholders = projectIdCandidates.map(() => '?').join(', ');
      await this.entityManager.query(
        `DELETE FROM aliados_contribuciones WHERE proyecto_id IN (${projectIdPlaceholders})`,
        projectIdCandidates
      );
      ...
      await this.syncPrimaryAllyContribution(
        id,
        projectData.allyName ?? projectData.nombreAliado ?? projectData.nombre_aliado,
        projectData.amountAllies,
        projectData.year
      );
    }
  ...
  private async syncProjectActivities(projectId: string, activities: any[] = []) {
      ...
      await this.entityManager.query(
        `INSERT INTO proyecto_actividades (id, proyecto_id, nombre, mes, estado, observaciones, source, manual_locked, created_at)
         ...
         ON DUPLICATE KEY UPDATE
           ...
           updated_at = NOW()`,
        [activityId, projectId, name, parsedMonth, normalizedStatus, note, 'manual', 1]
      );
    }
  }

  private async syncPrimaryAllyContribution(projectId: string, allyName: any, allyAmount: any, year: any) {
    ...
    await this.entityManager.query(
      `INSERT INTO aliados_contribuciones (id, proyecto_id, nombre_aliado, monto, \`a\u00f1o\`)
       ...
       ON DUPLICATE KEY UPDATE
         ...
         monto = VALUES(monto),
         \`a\u00f1o\` = VALUES(\`a\u00f1o\`)`,
      [`ally-${projectId}`, projectId, cleanName, amount, year || new Date().getFullYear()]
    );
  }

  private async deleteProjectFiles(urls: string[] = []) {
    ...
    for (const rawUrl of urls) {
      ...
      if (!trimmed || deleted.has(trimmed)) continue;
      deleted.add(trimmed);
      ...
      try {
        ...
        // Ignorar URLs externas o archivos ya eliminados.
      }
    }
  }

  private slugifySegment(value: any) {
    return (value ?? '')
      .toString()
      .trim()
      .normalize('NFD')
  ...
  private deleteDirectoryIfExists(targetPath: string, rootPath: string) {
    try {
      ...
      // Si la carpeta ya no existe o no se puede eliminar, no bloqueamos la operaci�n.
    }
    return false;
  }

  private async deleteProjectResourceFolders(projectId: string, projectInfo: any[]) {
    const uploadRoot = path.resolve(process.cwd(), 'uploads');
  ...
  private async deleteProjectCascade(projectId: string) {
    const projectInfo = await this.entityManager.query(
      `SELECT p.id, p.nombre_proyecto as projectName, e.año as year, o.categoria as category
       FROM proyectos p
       JOIN ediciones e ON p.edicion_id = e.id
    ...
    const photosRows = await this.entityManager.query(
      `SELECT url FROM fotos_proyecto WHERE proyecto_id IN (${placeholders})`,
      candidateIds
    );
    const documentRows = await this.entityManager.query(
      `SELECT url FROM proyecto_documentos WHERE proyecto_id IN (${placeholders})`,
      candidateIds
    );
    const reportRows = await this.entityManager.query(
      `SELECT fotos FROM reportes_mensuales WHERE proyecto_id IN (${placeholders})`,
      candidateIds
    );

    const fileUrls = [
      ...photosRows.map((row: any) => row.url).filter(Boolean),
      ...documentRows.map((row: any) => row.url).filter(Boolean),
      ...reportRows.flatMap((row: any) => this.safeParseJsonArray(row.fotos)),
    ];
  ...
  private async syncProjectMonthObservations(projectId: string, monthObservations: any = {}) {
    ...
    for (const item of entries) {
      await this.entityManager.query(
        `INSERT INTO proyecto_mes_observaciones (id, proyecto_id, mes, observaciones, created_at)
         VALUES (?, ?, ?, ?, NOW())
         ON DUPLICATE KEY UPDATE
           observaciones = VALUES(observaciones),
           updated_at = NOW()`,
        [`pom-${projectId}-${String(item.month).padStart(2, '0')}`, projectId, item.month, item.observations || null]
      );
    }
  }

  private async syncProjectMonthComments(projectId: string, monthComments: any = {}) {
    ...
    const entries = Array.isArray(monthComments)
      ? monthComments
          ...
          .filter((item: any) => item.month > 0 && item.comentario)
      : Object.entries(monthComments || {})
          .flatMap(([month, comments]: any) => {
    ...
    for (const item of entries) {
      counter++;
      await this.entityManager.query(
        `INSERT INTO proyecto_mes_comentarios (id, proyecto_id, mes, comentario, autor, created_at)
         VALUES (?, ?, ?, ?, ?, NOW())
         ON DUPLICATE KEY UPDATE
  ...
  private async syncProjectReports(projectId: string, reports: any[] = [], userId?: number) {
    await this.entityManager.query(`DELETE FROM reportes_mensuales WHERE proyecto_id = ?`, [projectId]);
    const projectInvestmentRow = await this.entityManager.query(
      `SELECT monto_fgk as amountFGK FROM proyectos WHERE id = ? LIMIT 1`,
      [projectId]
    );
    ...
    for (const rawReport of reports) {
      ...
      const metaFinancial = this.validateMetaFinancialAgainstFgk(
        rawReport?.metaFinancial ?? rawReport?.meta_financiera ?? rawReport?.expectedFinancial ?? rawReport?.expectedFinancialAmount ?? 0,
        projectAmountFGK,
        'seguimiento'
      ) || 0;
  ...
  async createProject(projectData: any, userId: number) {
    ...
    // Buscar edición
    const edicion = await this.entityManager.query(
      `SELECT id FROM ediciones WHERE año = ?`,
      [projectData.year || 2025]
    );
  ...
  async deleteProject(id: string, userId: number) {
    ...
    const existing = await this.entityManager.query(
      `SELECT p.id, o.categoria as category
       FROM proyectos p
       JOIN organizaciones o ON p.organizacion_id = o.id
       WHERE p.id = ?`,
      [id]
  ...
  // ==============================================
  // NUEVOS M�TODOS: OBTENER PROYECTO CON DETALLES
  // ==============================================
  ...
  async getProjectById(id: string): Promise<any> {
    ...
    // Obtener aliados
    const allies = await this.entityManager.query(
      `SELECT id, nombre_aliado as name, monto as amount, a\u00f1o as year, nota as note
       FROM aliados_contribuciones WHERE proyecto_id IN (${projectPlaceholders})`,
      projectIdCandidates
    );
    
    // Obtener participantes de formación
    const participants = await this.entityManager.query(
      `SELECT id, nombre as name, edad as age, genero as gender, telefono as phone, email, rol_cargo as role, estado_formacion as status, departamento as department
       FROM participantes_formacion WHERE proyecto_id IN (${projectPlaceholders})`,
      projectIdCandidates
    );
    
    // Obtener reportes mensuales
    const reports = await this.entityManager.query(
      `SELECT id, mes as month, COALESCE(anio, año) as year, avance_tecnico_real as realTechnical, 
       ...
       FROM reportes_mensuales WHERE proyecto_id IN (${projectPlaceholders}) ORDER BY COALESCE(anio, año) DESC, mes DESC`,
      projectIdCandidates
    );

    const activities = await this.entityManager.query(
      `SELECT id, proyecto_id, nombre as name, mes as month, estado as status, observaciones as observations, created_at as createdAt, updated_at as updatedAt
       FROM proyecto_actividades
    ...
    const normalizeScheduleStatus = (value: any) => {
      const status = (value ?? '').toString().trim().toLowerCase();
      if (status.includes('finaliz') || status === 'completed') return 'completed';
      if (status.includes('proceso') || status.includes('ejec') || status.includes('adelant') || status.includes('retras') || status.includes('tiempo') || status === 'active') return 'active';
      if (status.includes('pend') || status === 'pending') return 'pending';
      return 'pending';
    };
    ...
    if (reports.length > 0) {
      ...
      for (const [month, list] of grouped.entries()) {
        const statuses = list.map((item: any) => this.normalizeActivityStatus(item.status));
        if (statuses.every((status: string) => status === 'completed')) {
          progress[month - 1] = 'completed';
        } else if (statuses.some((status: string) => status === 'completed' || status === 'active')) {
    ...
    const photos = await this.entityManager.query(
      `SELECT url FROM fotos_proyecto WHERE proyecto_id IN (${projectPlaceholders}) ORDER BY created_at DESC`,
      projectIdCandidates
    );

    const documents = await this.entityManager.query(
      `SELECT tipo, url
    ...
    const monthObservationsRows = await this.entityManager.query(
      `SELECT mes, observaciones
       FROM proyecto_mes_observaciones
       WHERE proyecto_id IN (${projectPlaceholders})
       ORDER BY mes ASC`,
      projectIdCandidates
    );
    const monthCommentsRows = await this.entityManager.query(
      `SELECT id, mes, comentario, autor, created_at as createdAt, updated_at as updatedAt
       FROM proyecto_mes_comentarios
       WHERE proyecto_id IN (${projectPlaceholders})
       ORDER BY mes ASC, created_at ASC`,
      projectIdCandidates
    );
    const completionLetterDocs = documents.filter((doc: any) => doc.tipo === 'completion_letter').map((doc: any) => doc.url);
    const changeControlDocs = documents.filter((doc: any) => doc.tipo === 'change_control').map((doc: any) => doc.url);
    const monthlyTrackingDocs = documents.filter((doc: any) => doc.tipo === 'monthly_tracking').map((doc: any) => doc.url);
    const monthObservations = monthObservationsRows.reduce((acc: Record<number, string>, row: any) => {
      acc[Math.max(1, Math.min(12, parseInt(row.mes, 10) || 1))] = row.observaciones || '';
      return acc;
    }, {});
    const monthComments = monthCommentsRows.reduce((acc: Record<number, any[]>, row: any) => {
  ...
  // ==============================================
  // NUEVOS M�TODOS: REPORTES Y CRONOGRAMA
  // ==============================================
  ...
  async addReport(
    ...
    const projectAmountRow = await this.entityManager.query(
      `SELECT monto_fgk as amountFGK FROM proyectos WHERE id = ? LIMIT 1`,
      [projectId]
    );
    ...
    // Actualizar el progreso del proyecto con el �ltimo reporte
  ...
  async deleteReport(projectId: string, reportId: string, userId: number): Promise<any> {
    const existing = await this.entityManager.query(
      `SELECT id FROM reportes_mensuales WHERE id = ? AND proyecto_id = ?`,
      [reportId, projectId]
    );
    if (existing.length === 0) {
    ...
    await this.entityManager.query(
      `DELETE FROM reportes_mensuales WHERE id = ? AND proyecto_id = ?`,
      [reportId, projectId]
    );
  ...
  async updateProgressMatrix(
    projectId: string,
    progress: string[], // Array de 12 elementos con estados
    userId: number
  ): Promise<any> {
    const yearRow = await this.entityManager.query(
      `SELECT e.año as year FROM proyectos p JOIN ediciones e ON p.edicion_id = e.id WHERE p.id = ? LIMIT 1`,
      [projectId]
    );
    const projectYear = yearRow[0]?.year || 2025;

    // Guardar el progreso como JSON en una tabla o actualizar reportes individuales
  ...
  // ==============================================
  // M�TODO ADICIONAL: LISTAR PROYECTOS CON FILTROS
  // ==============================================

  async getAllProjects(filters?: { year?: number; category?: string; status?: string }) {
    let query = `
      SELECT 
        p.id,
        p.nombre_proyecto as name,
