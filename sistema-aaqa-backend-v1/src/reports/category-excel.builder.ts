...
type CategoryKind = 'ONG' | 'Community' | 'FIS';
...
export async function buildCategoryExcelWorkbook(params: {
  category: string;
  year?: number;
  ...
  const clamp = (value: number) => Math.max(0, Math.min(100, Number((Number.isFinite(value) ? value : 0).toFixed(2))));
  ...
  const historicalRows = Array.from(historicalByYear.values())
    ...
    .map((item) => ([
      ...
      item.allies,
      item.beneficiaries,
      clamp(avg(item.technical, 'value') || (item.technical.reduce((a: number, b: number) => a + b, 0) / Math.max(1, item.technical.length))),
      clamp(avg(item.financial, 'value') || (item.financial.reduce((a: number, b: number) => a + b, 0) / Math.max(1, item.financial.length))),
    ]));
  ...
  if (!existsSync(artifactToolPath)) {
