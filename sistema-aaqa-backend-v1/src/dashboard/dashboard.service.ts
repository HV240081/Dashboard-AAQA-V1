...
export class DashboardService {
  ...
  private normalizeMysqlDate(value: any): string | null {
  ...
  private async ensureCommunitySchema() {
    try {
      await this.entityManager.query(`ALTER TABLE adescos ADD COLUMN año INT NULL AFTER corte_id`);
    } catch {}
  }

  private getCommunityYearSequenceId(prefix: string, year: number, sequence: number) {
    return `${prefix}${year}-${String(sequence).padStart(2, '0')}`;
  }

  private parseCommunitySequence(id: any, prefix: string, year: number): number | null {
  ...
  private parseCommunityParticipantSequence(id: any, adescoId: string): number | null {
    const value = (id ?? '').toString().trim();
  ...
  private buildNextCommunityId(prefix: string, year: number, usedSequences: Set<number>) {
    let seq = 1;
    while (usedSequences.has(seq)) seq++;
    usedSequences.add(seq);
    return this.getCommunityYearSequenceId(prefix, year, seq);
  }

  private async ensureEditionExists(year: number) {
    const edition = await this.entityManager.query(
  ...
  private async isEditionAvailable(year: number) {
    const edition = await this.entityManager.query(
      `SELECT id FROM ediciones WHERE año = ? LIMIT 1`,
      [year]
    );
  ...
  async getGlobalData(year?: number) {
    let joinEdicion = '';
    let whereClause = '';
    const params: any[] = [];
  ...
  async getCategoriesData(year?: number) {
    let joinEdicion = '';
    let whereClause = '';
    const params: any[] = [];
  ...
  async getFormativeData(year?: number) {
  ...
  async getProjects(filters: { year?: number; category?: string }) {
    let query = `
      SELECT 
        p.id,
    ...
    // Si no hay proyectos, devolvemos array vacío
    if (results.length === 0) return [];

    // Obtener todos los participantes asociados a estos proyectos en una sola consulta eficiente
    const projectIdCandidatesByCanonical = new Map<string, string[]>();
    ...
    const reports = await this.entityManager.query(
      `SELECT id, proyecto_id, mes, año as year, avance_tecnico_real as realTechnical,
    ...
    const activities = await this.entityManager.query(
      `SELECT id, proyecto_id, nombre as name, mes as month, estado as status, nota as note, created_at as createdAt, updated_at as updatedAt
    ...
    const projectPhotos = await this.entityManager.query(
      `SELECT proyecto_id, url, created_at
       FROM fotos_proyecto
       WHERE proyecto_id IN (${placeholders})
    ...
    const projectDocuments = await this.entityManager.query(
      `SELECT proyecto_id, tipo, url, created_at
    ...
    const completionDocsByProject = new Map<string, string[]>();
    const changeDocsByProject = new Map<string, string[]>();
    const activitiesByProject = new Map<string, any[]>();
    const normalizeReportStatus = (value: any): 'pending' | 'active' | 'completed' => {
      const status = (value ?? '').toString().trim().toLowerCase();
      if (status.includes('finaliz')) return 'completed';
      if (status.includes('tiempo') || status.includes('ejec') || status.includes('adelant') || status.includes('retras')) return 'active';
      return 'pending';
    };

    for (const report of reports) {
      const reportKey =
        candidateToCanonical.get(report.proyecto_id.toString()) ||
    ...
    for (const photo of projectPhotos) {
      const photoKey =
        candidateToCanonical.get(photo.proyecto_id.toString()) ||
    ...
    for (const doc of projectDocuments) {
      const docKey =
        candidateToCanonical.get(doc.proyecto_id.toString()) ||
    ...
    for (const activity of activities) {
      const activityKey =
        candidateToCanonical.get(activity.proyecto_id.toString()) ||
    ...
    // Agrupar participantes por proyecto_id
    ...
    for (const p of participants) {
      ...
      const participantProjectKey =
        candidateToCanonical.get(participantProjectId) ||
    ...
    return Promise.all(results.map(async row => {
      ...
      if (projParts.length === 0) {
        ...
        const categoryParams = normalizedCategory === 'Community'
          ...
          : [normalizedCategory];
        const fallbackParticipants = await this.entityManager.query(
          `SELECT 
             pf.id,
             pf.proyecto_id,
             pf.nombre as name,
           ...
           WHERE p2.nombre_proyecto = ?
             AND ${categoryWhere}`,
          [row.name, ...categoryParams]
        );

        projParts = fallbackParticipants.map((p: any) => ({
          id: p.id,
          name: p.name,
          age: p.age,
          gender: p.gender,
  ...
  async getCommunityCortes() {
    ...
    for (const p of participants) {
      if (!partsByAdesco.has(p.adesco_id)) partsByAdesco.set(p.adesco_id, []);
    ...
    for (const a of adescos) {
      ...
      if (!adescosByCorte.has(a.corte_id)) adescosByCorte.set(a.corte_id, []);
  ...
  async updateCommunityCortes(cortes: any[]) {
    await this.ensureCommunitySchema();
    const usedCorteSequencesByYear = new Map<number, Set<number>>();
    const usedAdescoSequencesByYear = new Map<number, Set<number>>();
    const usedParticipantSequencesByAdesco = new Map<string, Set<number>>();
    const warnings: string[] = [];
    const validCortes: any[] = [];
    ...
    return this.entityManager.transaction(async transactionalEntityManager => {
      ...
      for (const corte of validCortes) {
        ...
        if (!usedCorteSequencesByYear.has(corteYear)) {
          usedCorteSequencesByYear.set(corteYear, new Set<number>());
        }
        ...
        await transactionalEntityManager.query(
          `INSERT INTO cortes_comunitarios (id, año, nombre, ubicacion, nombre_aliado, estado, fecha_inicio, fecha_fin)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
          [
            finalCorteId,
            corteYear,
        ...
        if (corte.adescos && corte.adescos.length > 0) {
          for (const adesco of corte.adescos) {
            ...
            if (!usedAdescoSequencesByYear.has(adescoYear)) {
              usedAdescoSequencesByYear.set(adescoYear, new Set<number>());
            }
            ...
            await transactionalEntityManager.query(
              `INSERT INTO adescos (id, corte_id, año, nombre, participantes_inscritos, participantes_graduados, mujeres, hombres)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
              [
                finalAdescoId,
                finalCorteId,
            ...
            if (adesco.participants && adesco.participants.length > 0) {
              if (!usedParticipantSequencesByAdesco.has(finalAdescoId)) {
                usedParticipantSequencesByAdesco.set(finalAdescoId, new Set<number>());
              }
              ...
              for (const part of adesco.participants) {
                ...
                await transactionalEntityManager.query(
                  `INSERT INTO adescos_participantes (id, adesco_id, nombre, cargo, contacto, distrito, departamento, genero)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
                  [
                    finalParticipantId,
                    finalAdescoId,
  ...
  async deleteCommunityCorte(corteId: string) {
    const id = (corteId ?? '').toString().trim();
    if (!id) {
      throw new Error('ID de corte inválido.');
    }

    const corte = await this.entityManager.query(
      `SELECT id FROM cortes_comunitarios WHERE id = ? LIMIT 1`,
      [id]
    );
    ...
    await this.entityManager.transaction(async transactionalEntityManager => {
      ...
      await transactionalEntityManager.query(
        `DELETE FROM adescos WHERE corte_id = ?`,
        [id]
      );
      await transactionalEntityManager.query(
        `DELETE FROM cortes_comunitarios WHERE id = ?`,
        [id]
      );
    });
  ...
  async getFisParticipants(year?: number) {
    let incubadoraQuery = `
      SELECT 
        id,
        año as year,
  ...
  async replaceFisParticipants(participants: any[], userId?: number) {
    ...
    return this.entityManager.transaction(async transactionalEntityManager => {
      ...
      const trainingProjectIds = Array.from(new Set(
        list
      ...
      if (trainingProjectIds.length > 0) {
        ...
        await transactionalEntityManager.query(
          `DELETE FROM participantes_formacion WHERE proyecto_id IN (${placeholders})`,
          trainingProjectIds
        );
      }
      ...
      for (const participant of list) {
        ...
        if (participant?.source === 'training' && participant?.projectId) {
          await transactionalEntityManager.query(
            `INSERT INTO participantes_formacion (id, proyecto_id, nombre, edad, genero, telefono, email, rol_cargo, estado_formacion, departamento, municipio, fecha_inscripcion)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())
          ...
          await transactionalEntityManager.query(
            `INSERT INTO participantes_incubadora (id, año, programa, sede, nombre, genero, edad, nombre_emprendimiento, departamento, estado, observaciones, created_at)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())
             ON DUPLICATE KEY UPDATE
               año = VALUES(año),
  ...
  async deleteFisParticipant(participantId: string, userId?: number) {
    ...
    const incubadoraExisting = await this.entityManager.query(
      `SELECT id FROM participantes_incubadora WHERE id = ?`,
      [id]
    );
    const trainingExisting = await this.entityManager.query(
      `SELECT pf.id, pf.proyecto_id
       FROM participantes_formacion pf
       INNER JOIN proyectos p ON pf.proyecto_id = p.id OR pf.proyecto_id = SUBSTRING_INDEX(p.id, '-', 1)
       INNER JOIN organizaciones o ON p.organizacion_id = o.id
       WHERE pf.id = ? AND o.categoria = 'FIS'`,
      [id]
    );
    ...
    if (incubadoraExisting.length > 0) {
      await this.entityManager.query(
        `DELETE FROM participantes_incubadora WHERE id = ?`,
        [id]
      );
    }
    ...
    if (trainingExisting.length > 0) {
      await this.entityManager.query(
        `DELETE FROM participantes_formacion WHERE id = ?`,
        [id]
      );
    }
  ...
  // ==============================================
  // NUEVOS MÉTODOS DE GEOGRAFÍA Y TEXTOS
  // ==============================================

  async getDepartamentos(): Promise<string[]> {
    const result = await this.entityManager.query(
        `SELECT DISTINCT departamento FROM referencias_geograficas ORDER BY departamento`
    );
    return result.map(r => r.departamento);
  }

  async getMunicipios(departamento: string): Promise<string[]> {
    const result = await this.entityManager.query(
        `SELECT DISTINCT municipio FROM referencias_geograficas 
         WHERE departamento = ? ORDER BY municipio`,
        [departamento]
    );
    return result.map(r => r.municipio);
  }

  async getDistritos(departamento: string, municipio: string): Promise<string[]> {
    const result = await this.entityManager.query(
        `SELECT DISTINCT distrito FROM referencias_geograficas 
         WHERE departamento = ? AND municipio = ? ORDER BY distrito`,
        [departamento, municipio]
    );
    return result.map(r => r.distrito);
  }

  async getTextos(categoria?: string): Promise<Record<string, string>> {
  ...
  async updateTexto(
    ...
    await this.entityManager.query(
        `INSERT INTO textos_editables (categoria, clave, valor, actualizado_por, actualizado_en)
         VALUES (?, ?, ?, ?, NOW())
         ON DUPLICATE KEY UPDATE 
            valor = VALUES(valor),
            actualizado_por = VALUES(actualizado_por),
            actualizado_en = NOW()`,
        [categoria, clave, valor, usuarioId]
    );
  ...
  async getAvailableYears(): Promise<number[]> {
    const editionYears = await this.entityManager.query(
        `SELECT DISTINCT año FROM ediciones WHERE año BETWEEN 2009 AND 2100`
    );

    const projectYears = await this.entityManager.query(
        `SELECT DISTINCT e.año
         FROM proyectos p
         JOIN ediciones e ON p.edicion_id = e.id
    ...
    for (const row of editionYears) years.add(parseInt(row.año, 10));
    for (const row of projectYears) years.add(parseInt(row.año, 10));
  ...
  async createEdition(year: number, status: string = 'ABIERTA'): Promise<any> {
    // Verificar si ya existe
    const exists = await this.entityManager.query(
        `SELECT id FROM ediciones WHERE año = ?`, [year]
    );
    ...
    const maxYearResult = await this.entityManager.query(
        `SELECT MAX(año) AS maxYear FROM ediciones`
    );
    ...
    await this.entityManager.query(
        `INSERT INTO ediciones (año, estado, es_actual)
         VALUES (?, ?, ?)`,
        [year, status, shouldBeCurrent]
    );

    return { success: true, message: `Edición ${year} creada correctamente.` };
  }

  async deleteEdition(year: number): Promise<any> {
    const exists = await this.entityManager.query(
        `SELECT id FROM ediciones WHERE año = ?`, [year]
    );
    ...
    // Eliminar proyectos asociados a esta edición primero para evitar problemas de Foreign Key
    await this.entityManager.query(
        `DELETE FROM proyectos WHERE edicion_id = ?`, [edicionId]
    );

    // Luego eliminar la edición
    await this.entityManager.query(
        `DELETE FROM ediciones WHERE id = ?`, [edicionId]
    );

    const remainingLatestEdition = await this.entityManager.query(
        `SELECT id FROM ediciones ORDER BY año DESC LIMIT 1`
    );

    if (remainingLatestEdition.length > 0) {
        await this.entityManager.query(
            `UPDATE ediciones SET es_actual = FALSE`
        );
        await this.entityManager.query(
            `UPDATE ediciones SET es_actual = TRUE WHERE id = ?`,
            [remainingLatestEdition[0].id]
        );
    }
  ...
  async getGeoData(year?: number): Promise<any[]> {
    ...
    // Enriquecer con coordenadas desde referencias_geograficas
    const geoData = [];
    for (const row of results) {
        const coords = await this.entityManager.query(
            `SELECT latitud, longitud FROM referencias_geograficas 
             WHERE departamento = ? AND municipio = ? AND distrito = ?
             LIMIT 1`,
            [row.departamento, row.municipio, row.distrito]
        );
  ...
  async getStatsByDepartamento(year?: number): Promise<any[]> {
    let joinEdicion = '';
    let whereClause = '';
    const params: any[] = [];
  ...
  async compareYears(years: number[]): Promise<any> {
    const results: any[] = [];
  ...
  async getCommunityCounterpart(projectId: string): Promise<any> {
    const result = await this.entityManager.query(
        `SELECT contrapartida_org as laborAmount, 0 as materialsAmount 
  ...
  async updateCommunityCounterpart(
    projectId: string, 
    data: { laborAmount: number; materialsAmount: number },
    usuarioId: number
  ): Promise<any> {
    const totalCounterpart = data.laborAmount + data.materialsAmount;
    
    await this.entityManager.query(
        `UPDATE proyectos SET contrapartida_org = ?, updated_at = NOW() WHERE id = ?`,
        [totalCounterpart, projectId]
    );
  ...
  // ==============================================
  // MÉTODOS DE ESCRITURA (PUT) - CORREGIDOS
  // ==============================================

  async updateFinancials(data: { fgk: number; aliados: number; contrapartida: number }) {
    try {
      // Intentar guardar en tabla de configuración
      await this.entityManager.query(`
      ...
      // Si la tabla no existe, solo registramos
  ...
  async updateFormative(data: any) {
    try {
      const queries = [];
      const params = [];
  ...
  async updateCurrentEdition(data: any) {
  ...
  async getTextContent() {
    try {
      const texts = await this.entityManager.query(
        `SELECT seccion, campo, valor FROM configuracion_manual WHERE seccion LIKE 'text_%' ORDER BY actualizado_en ASC`
      );
  ...
  async updateTextContent(category: string, key: string, value: string) {
    try {
      await this.entityManager.query(`
        INSERT INTO configuracion_manual (seccion, campo, valor, actualizado_en)
        VALUES (?, ?, ?, NOW())
  ...
  async getEditions() {
    return this.entityManager.query(
      `SELECT id, año as year, estado as status, es_actual as isCurrent FROM ediciones ORDER BY año DESC`
    );
  }
}
