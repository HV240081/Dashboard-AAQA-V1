...
export class AuditInterceptor implements NestInterceptor {
  ...
  private actionFromMethod(method: string): 'INSERT' | 'UPDATE' | 'DELETE' {
    if (method === 'DELETE') return 'DELETE';
    if (method === 'PUT') return 'UPDATE';
    return 'INSERT';
  }

  private resolveAuditTarget(request: any): AuditTarget | null {
    const method = request.method;
    const url = (request.url || '').split('?')[0];
    const parts = url.split('/').filter(Boolean);

    const at = (segment: string) => parts.indexOf(segment);
  ...
  private async fetchCurrentData(target: AuditTarget) {
    if (!target.oldQuery || !target.oldParams?.length) return null;
  ...
  private diffObjects(oldData: any, newData: any) {
  ...
  private publicPayload(request: any, responseData: any) {
  ...
  private resolveFinalRecordId(target: AuditTarget, responseData: any) {
